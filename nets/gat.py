...
class GraphAttention(nn.Module):
    def __init__(self, g, in_dim, out_dim,  num_heads, feat_drop, attn_drop,
                alpha, residual=False, freeze=0, name=None):
        ...
        self.fc = nn.Linear(in_dim+self.freeze, num_heads * out_dim, bias=False)
        if feat_drop:
    ...
    def forward(self, inputs):
        ...
        if self.freeze > 0:
            ft = torch.cat((self.g.ndata['frozen'], h), 1)
            # print(self.name, 'after   ', ft[0][:10])
            ft = self.fc(ft).reshape((h.shape[0], self.num_heads, -1))  # NxHxD'
        else:
